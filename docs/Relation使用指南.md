...
## Sprint / 多值枚举分组模式（Table）

当你在 Table 里按“多值枚举字段”（如 `sprint`）分组时，现在支持两种模式：

- `拆分（split）`：一条记录会出现在它所属的每个 Sprint 分组中。
  - 适合跨 Sprint 协作、发布列车、并行迭代场景。
- `仅首值（first）`：只使用列表第一个值作为分组。
  - 适合强调“唯一主归属 Sprint”的执行看板。

建议：

- 团队同时有“统计视角 + 执行视角”时，默认用 `split`；
- 若需要严格做到“一行一个 Sprint”，切换为 `first`，并约定把主 Sprint 放在第一位。
